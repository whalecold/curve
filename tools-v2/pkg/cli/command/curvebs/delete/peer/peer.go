...
func (cCmd *Command) Init(cmd *cobra.Command, args []string) error {
	...
	// parse peers config
	curConf := config.GetBsFlagString(cCmd.Cmd, config.CURVEBS_PEERS_ADDRESS)
	...
	// parse peer conf
	if len(args) < 1 {
		return fmt.Errorf("should specified the peer address")
	}
	peer := args[0]
...
func (cCmd *Command) RunCommand(cmd *cobra.Command, args []string) error {
	...
	defer func() {
		...
		out[cobrautil.ROW_RESULT] = string(byteRet)
