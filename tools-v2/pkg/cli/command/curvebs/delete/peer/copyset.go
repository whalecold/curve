...
func DeleteBrokenCopyset(logicalPoolID, copysetID uint32, peer *common.Peer, opts Options) error {
	...
	if errCmd.TypeCode() != cmderror.CODE_SUCCESS {
		return fmt.Errorf("failed to delete broken copyset %s error : %s", peer.GetAddress(), errCmd.Message)
	}
	resp, ok := response.(*copyset.CopysetResponse)
	if !ok {
		return fmt.Errorf("error type interface when acquire delete broken copyset info %s", peer.GetAddress())
	}
	if resp.GetStatus() != copyset.COPYSET_OP_STATUS_COPYSET_OP_STATUS_SUCCESS {
		return fmt.Errorf("delete broken failed, error code %v", resp.GetStatus())
	}
	return nil
}
